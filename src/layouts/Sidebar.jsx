...
const Sidebar = ({ user, isToggle, setToggle }) => {
   ...
   const adminLinks = [
      ...
      { name: "Notifications", path: "admin/push-notifications", icon: <FaBell /> },
   ...
   return (
      <>
         ...
         <aside
            className={
               `fixed top-0 left-0 z-20 w-56 h-screen bg-[#E67002] transition-all duration-300 ${isToggle ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
               }`}
         >
                  ...
                  <ul>
                     {links.map((link) => (
                        <li key={link.path}>
                           <Link
                              to={link.path}
                              className={`flex items-center text-sm font-medium px-4 py-3 mx-2 rounded-md transition-colors ${location.pathname.includes(link.path)
                                 ? 'bg-[#c4650d]'
