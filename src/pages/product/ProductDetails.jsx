...
const ProductDetails = () => {
	...
	const handleAddToCart = async () => {
		...
		const hasUnselectedOptions = product.options
			?.filter((option) => option.values?.length)
			.some((option) => !selectedOptions[option.name]);
	...
	const handleShare = () => {
		if (navigator.share) {
			navigator
				.share({
					title: product.name,
					text: `Check out ${product.name} on Hezmart!`,
			...
			const shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(
				`Check out ${product.name} on Hezmart!`,
	...
	const discountPercentage = product.isOnFlashSale
		? Math.round(
			(1 - parseFloat(product.displayPrice) / parseFloat(product.price)) *
			100,
		)
	...
	const reviewsSection = <ReviewComponent product={product} user={user} />;
	...
	return (
		<div className="max-w-7xl mx-auto min-h-screen">
					...
					<div className="flex gap-3 overflow-x-auto pb-2">
						{allImages.map((img, index) => (
							<button
								...
								className={`w-20 h-20 rounded-full overflow-hidden border transition ${selectedImage === index
				...
				<div className="flex flex-col gap-8">
					...
					<div className="flex items-center gap-2">
						...
						{discountPercentage > 0 && (
							<>
								...
								<span
									className={`text-xs px-3 py-1 rounded-full border ${product.isOnFlashSale
								...
								<div className="flex flex-wrap gap-2">
									{option.values.map((value) => (
										<button
											...
											className={`px-4 py-1 rounded-full text-sm border transition ${selectedOptions[option.name] === value.value
													? "border-black"
					...
					<div className="flex flex-col gap-4">
						<Button
							...
							loadingText="Adding..."
							className={`w-full py-3 font-medium text-base rounded-full ${product.isOnFlashSale ? "bg-red-600 hover:bg-red-700" : ""
								}`}
						>
			...
			{/* Similar Products */}
			<div>
				<h2 className="text-xl md:text-2xl font-semibold mb-4 font-['Poppins']">
