...
const Checkout = () => {
    const { cart, updateQuantity, removeFromCart, clearCart, fetchCart } =
        useCart();
    ...
    const getShippingSettings = async () => {
        try {
            const res = await axios.get(
                'api/v1/shipping-settings/active'
            );
            if (res.data.status === 'success') {
            ...
            const message =
                err.response?.data?.message ||
                'Failed to fetch shipping settings';
            toast.error(message);
        }
    };
    ...
    const handleCheckout = async (selectedShipping) => {
        ...
        const allAvailable =
            cart.items?.every(item => item.available) ?? false;
        if (!allAvailable) {
            toast.error(
        ...
        try {
            const payload = {
                ...
                paymentMethod,
                ...(deliveryOption === 'door' && {
                    selectedStateId: selectedStateFee.id,
                }),
                ...(deliveryOption === 'pickup' && {
            ...
            if (
                ...
                navigate('/orders');
                toast.success(
                    'Please make payment to crypto wallet'
                );
            }
        } catch (error) {
                    ...
                    <div className="text-red-500">
                        ...
                        >
                            <path
                                ...
                                1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                                clipRule="evenodd"
                            />
                        </svg>
                    </div>
    ...
    const { items = [], summary = {}, shippingOptions = [] } =
        cart;
    const allItemsAvailable = items.every(
        item => item.available
    );
    ...
    return (
        <div className="max-w-7xl mx-auto py-8">
            <div className="flex items-center mb-6">
                <h1 className="text-xl lg:text-2xl font-semibold font-['poppins'] text-gray-900">
                    Checkout
                </h1>
            </div>
