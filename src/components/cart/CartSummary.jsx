...
import { FiCopy, FiTag, FiAlertCircle, FiCheckCircle, FiUser, FiMapPin, FiPhone, FiMail, FiPlus, FiMinus } from 'react-icons/fi';
                  ...
                  <div className="border-t border-gray-100 pt-4 mt-4 space-y-4">
                        ...
                        <div className="space-y-3">
                           <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                 <div>
                                    <p className="text-xs font-bold text-gray-900 leading-none">Offset with Earnings</p>
                                    <p className="text-[10px] text-gray-500 mt-1">Available: ₦{(currentUser?.bonus || 0).toLocaleString()}</p>
                                 </div>
                              </div>
                              <button
                                 onClick={() => {
                                    setUseOffset(!useOffset);
                                    if (!useOffset) setOffsetAmount(500);
                                    else setOffsetAmount(0);
                                 }}
                                 className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${useOffset ? 'bg-primary-light' : 'bg-gray-200'}`}
                              >
                                 <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${useOffset ? 'translate-x-4' : 'translate-x-0'}`} />
                              </button>
                           </div>
                           ...
                           {useOffset && (
                              <div className="flex items-center justify-between bg-white rounded-xl p-2 border border-orange-100">
                                 <button
                                    onClick={() => setOffsetAmount(prev => Math.max(500, prev - 500))}
                                    disabled={offsetAmount <= 500}
                                    className="p-1.5 text-orange-600 hover:bg-orange-50 rounded-md cursor-pointer disabled:opacity-30 transition-colors"
                                 >
                                    <FiMinus size={16} />
                                 </button>
                                 <div className="text-center px-4">
                                    <p className="text-sm font-extrabold text-orange-700">- ₦{offsetAmount.toLocaleString()}</p>
                                 </div>
                                 <button
                                    onClick={() => setOffsetAmount(prev => Math.min(currentUser?.bonus || 0, Math.floor(summary.total + (deliveryFee || 0)), prev + 500))}
                                    disabled={offsetAmount >= (currentUser?.bonus || 0) || offsetAmount >= Math.floor(summary.total + (deliveryFee || 0))}
                                    className="p-1.5 text-orange-600 hover:bg-orange-50 rounded-md cursor-pointer disabled:opacity-30 transition-colors"
                                 >
                                    <FiPlus size={16} />
                                 </button>
                     ...
                     <div className="flex justify-between items-end gap-4">
                        ...
                        <div className="text-2xl font-extrabold text-gray-900 
                           ...
                           ₦{(summary.total + (deliveryFee || 0) - (useOffset ? offsetAmount : 0)).toLocaleString()}
                        </div>
                     </div>
                        ...
                        <div className="rounded-xl bg-red-50 p-3 border border-red-100">
                           <div className="flex">
                              <div className="flex items-center gap-2 ml-3">
                                 <FiAlertCircle className="h-4 w-4 text-red-800" aria-hidden="true" />
                                 <span className="text-xs font-medium text-red-800">Cannot proceed yet</span>
