...
const DeliveryOptionSection = ({ 
  ...
  const handleStateChange = (e) => {
    ...
    if (deliveryOption === 'door') {
      ...
      const filteredStations = allPickupStations.filter(station => station.state === newState);
  ...
  // const handleStateFeeSelect = state=>{
  //   setSelectedStateFee(state)
  // }
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className='flex justify-between mb-4'>
          <div className='flex gap-3 items-center'>
            <div className="h-6 w-6 rounded-full flex items-center justify-center bg-gray-300 text-white">
              <IoCheckmarkSharp />
            </div>
        ...
        <p className="text-gray-500">No delivery options available at this time</p>
      </div>
    );
  }

  return (
    <div className={`bg-white rounded-lg shadow-sm p-6 ${isStepCompleted ? 'border-l-4 border-green-500' : ''}`}>
      <div className='flex justify-between mb-4'>
        <div className='flex gap-3 items-center'>
          <div className={`h-6 w-6 rounded-full flex items-center justify-center text-white ${isStepCompleted ? 'bg-green-500' : 'bg-gray-300'}`}>
            {isStepCompleted && <IoCheckmarkSharp />}
          </div>
          <h1 className='uppercase text-sm font-semibold'>
            2. Delivery Option
          </h1>
        ...
        {doorDeliveryEnabled && pickupEnabled ? (
          <div className="flex gap-4">
            <button
              ...
              className={`px-4 py-3 border rounded-md flex-1 text-left ${deliveryOption === 'door' ? 'border-green-500 bg-green-50' : 'border-gray-300'}`}
            >
              <div className="flex items-center gap-2">
                <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${deliveryOption === 'door' ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300'}`}>
                  {deliveryOption === 'door' && <IoCheckmarkSharp size={14} />}
                </div>
            ...
            <button
              ...
              className={`px-4 py-3 border rounded-md flex-1 text-left ${deliveryOption === 'pickup' ? 'border-green-500 bg-green-50' : 'border-gray-300'}`}
            >
              <div className="flex items-center gap-2">
                <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${deliveryOption === 'pickup' ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300'}`}>
                  {deliveryOption === 'pickup' && <IoCheckmarkSharp size={14} />}
                </div>
          ...
          <div className={`px-4 py-3 border rounded-md ${isStepCompleted ? 'border-green-500 bg-green-50' : 'border-gray-300'}`}>
            <div className="flex items-center gap-2">
              <div className={`h-5 w-5 rounded-full border flex items-center justify-center ${isStepCompleted ? 'border-green-500 bg-green-500 text-white' : 'border-gray-300'}`}>
          ...
          <div className="p-4 border border-gray-200 rounded-md bg-gray-50 mt-4">
            <div className="mb-3">
              ...
              <select
                ...
                className="w-full py-2 px-4 pr-10 transition-all duration-200 focus:outline-none appearance-none border border-[#D9E1EC] rounded-lg placeholder-[#A1A7C4] text-black focus:border-primary-light"
              >
                <option value="">Select State</option>
                {stateFees.map(fee => (
                  <option  key={fee.id} value={fee.state}>{fee.state}</option>
                ))}
              </select>
            </div>
            {internalSelectedState && (
              <div className="text-sm">
                <p className="font-medium">Delivery Fee: ₦{deliveryFee?.toLocaleString()}</p>
              </div>
            )}
          </div>
        )}
        ...
        {deliveryOption === 'pickup' && pickupEnabled && (
          <div className="p-4 border border-gray-200 rounded-md bg-gray-50 mt-4">
            <div className="mb-3">
              ...
              >
                <option value="">Select State</option>
                {Array.from(new Set(allPickupStations.map(station => station.state))).map(state => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <div className={`mt-1 h-4 w-4 rounded-full border flex items-center justify-center ${
                          ...
                          {selectedPickupStation === station.id && <IoCheckmarkSharp size={10} className="text-white" />}
                        </div>
                        <div>
                          <h4 className="font-medium">{station.name}</h4>
                          <p className="text-sm text-gray-600">{station.address}</p>
                          <p className="text-sm text-gray-600">Phone: {station.contactPhone}</p>
                          <p className="text-sm font-medium mt-1">Fee: ₦{station.fee?.toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                  ))}
