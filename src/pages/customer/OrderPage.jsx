   ...
   useEffect(() => {
      const fetchOrders = async () => {
         try {
            ...
            // If a payment reference is present, verify it first to trigger order creation
            if (reference) {
               try {
                  await axios.get(`api/v1/orders/verify-payment/${reference}`);
                  toast.success('Payment verified successfully!');
                  // Clear parameters from URL after successful verification
                  setSearchParams({}, { replace: true });
               } catch (err) {
                  // If order already exists or verification fails, we still want to show the orders list
